...
public class DocumentContextFactoryTest : LanguageServerTestBase
{
    ...
    public async Task TryCreateAsync_ResolvesContent()
    {
        ...
        {
            updater.DocumentAdded(MiscFilesProject.HostProject.Key, hostDocument, TestMocks.CreateTextLoader(filePath, ""));
        });
    ...
    public async Task TryCreateAsync_WithProjectContext_Resolves()
    {
        ...
        await _projectManager.UpdateAsync(updater =>
        {
            ...
            updater.DocumentAdded(hostProject.Key, hostDocument, EmptyTextLoader.Instance);
        });
