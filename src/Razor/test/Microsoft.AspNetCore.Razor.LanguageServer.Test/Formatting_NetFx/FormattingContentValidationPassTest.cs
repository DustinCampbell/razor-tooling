...
public class FormattingContentValidationPassTest(ITestOutputHelper testOutput) : LanguageServerTestBase(testOutput)
{
    ...
    private static FormattingContext CreateFormattingContext(TestCode input, int tabSize = 4, bool insertSpaces = true, string? fileKind = null)
    {
        ...
        var (document, codeDocument) = CreateDocumentAndGeneratedOutput(source, uri.AbsolutePath, fileKind: fileKind);
    ...
    private static (IRazorDocument, RazorCodeDocument) CreateDocumentAndGeneratedOutput(SourceText text, string path, ImmutableArray<TagHelperDescriptor> tagHelpers = default, string? fileKind = null)
    {
