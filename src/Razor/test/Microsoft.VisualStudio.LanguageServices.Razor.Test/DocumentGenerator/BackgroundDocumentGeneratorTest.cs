...
public class BackgroundDocumentGeneratorTest(ITestOutputHelper testOutput) : VisualStudioWorkspaceTestBase(testOutput)
{
    ...
    private static readonly IFallbackProjectManager s_fallbackProjectManager =
        StrictMock.Of<IFallbackProjectManager>(m => m.IsFallbackProject(It.IsAny<IProjectSnapshot>()) == false);
    ...
    private sealed class TestDynamicFileInfoProvider : IRazorDynamicFileInfoProviderInternal
    {
