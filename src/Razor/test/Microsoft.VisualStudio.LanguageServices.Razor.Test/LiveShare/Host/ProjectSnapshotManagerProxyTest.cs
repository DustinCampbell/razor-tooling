...
public class ProjectSnapshotManagerProxyTest(ITestOutputHelper testOutput) : VisualStudioTestBase(testOutput)
{
    ...
    public async Task CalculateUpdatedStateAsync_ReturnsStateForAllProjects()
    {
        ...
        // Assert
        var project1TagHelpers = await projectManager.GetRequiredProject(_hostProject1.Key).GetTagHelpersAsync(DisposalToken);
        var project2TagHelpers = await projectManager.GetRequiredProject(_hostProject2.Key).GetTagHelpersAsync(DisposalToken);
    ...
    public async Task Changed_TriggersOnSnapshotManagerChanged()
    {
        ...
        await projectManager.UpdateAsync(updater =>
        {
            ...
            updater.UpdateProjectConfiguration(_hostProject1.Key, FallbackRazorConfiguration.MVC_1_0);
        });
