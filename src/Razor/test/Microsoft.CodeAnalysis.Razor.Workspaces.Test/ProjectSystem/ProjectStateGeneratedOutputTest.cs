...
public class ProjectStateGeneratedOutputTest(ITestOutputHelper testOutput) : WorkspaceTestBase(testOutput)
{
    ...
    public async Task DocumentAdded_DoesNotCacheOutput()
    {
        ...
        var state = ProjectState
            .Create(ProjectEngineFactoryProvider, LanguageServerFeatureOptions, s_project, ProjectWorkspaceState.Default)
            .WithAddedHostDocument(s_document1, DocumentState.EmptyLoader);

        var result = await GetOutputAsync(state, s_document1, DisposalToken);

        // Act
        var newState = state.WithAddedHostDocument(s_document2, DocumentState.EmptyLoader);
    ...
    public async Task DocumentChanged_DoesNotCacheOutput()
    {
        ...
        var original = ProjectState
            .Create(ProjectEngineFactoryProvider, LanguageServerFeatureOptions, s_project, ProjectWorkspaceState.Default)
            .WithAddedHostDocument(s_document1, DocumentState.EmptyLoader)
            .WithAddedHostDocument(s_importDocument, DocumentState.EmptyLoader);

        var result = await GetOutputAsync(original, s_document1, DisposalToken);

        // Act
        var version = VersionStamp.Create();
        var newState = original.WithChangedHostDocument(s_document1, TestMocks.CreateTextLoader("@using System", version));
        var newResult = await GetOutputAsync(newState, s_document1, DisposalToken);
    ...
    public async Task DocumentRemoved_DoesNotCacheOutput()
    {
        ...
        var state = ProjectState
            .Create(ProjectEngineFactoryProvider, LanguageServerFeatureOptions, s_project, ProjectWorkspaceState.Default)
            .WithAddedHostDocument(s_document1, DocumentState.EmptyLoader)
            .WithAddedHostDocument(s_document2, DocumentState.EmptyLoader);

        var result = await GetOutputAsync(state, s_document1, DisposalToken);

        // Act
        var newState = state.WithRemovedHostDocument(s_document2);
        var newResult = await GetOutputAsync(newState, s_document1, DisposalToken);
    ...
    public async Task ImportDocumentAdded_DoesNotCacheOutput()
    {
        ...
        // Act
        var newState = state.WithAddedHostDocument(s_importDocument, DocumentState.EmptyLoader);
        var newResult = await GetOutputAsync(newState, s_document1, DisposalToken);
    ...
    public async Task ImportDocumentChanged_DoesNotCacheOutput()
    {
        ...
        // Act
        var version = VersionStamp.Create();
        var newState = state.WithChangedHostDocument(s_importDocument, TestMocks.CreateTextLoader("@using System", version));
        var newResult = await GetOutputAsync(newState, s_document1, DisposalToken);
    ...
    public async Task ImportDocumentRemoved_DoesNotCacheOutput()
    {
        ...
        // Act
        var newState = state.WithRemovedHostDocument(s_importDocument);
        var newResult = await GetOutputAsync(newState, s_document1, DisposalToken);
    ...
    public async Task ProjectWorkspaceStateChange_WithProjectWorkspaceState_CSharpLanguageVersion_DoesNotCacheOutput()
    {
        ...
        var state = ProjectState
            .Create(ProjectEngineFactoryProvider, LanguageServerFeatureOptions, hostProject, ProjectWorkspaceState.Create(s_someTagHelpers, LanguageVersion.CSharp7))
            .WithAddedHostDocument(s_document1, TestMocks.CreateTextLoader("@DateTime.Now", VersionStamp.Default));
