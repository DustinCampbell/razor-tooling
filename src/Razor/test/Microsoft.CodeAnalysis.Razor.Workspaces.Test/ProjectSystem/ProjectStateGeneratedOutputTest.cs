...
public class ProjectStateGeneratedOutputTest : WorkspaceTestBase
{
    ...
    private static ValueTask<OutputAndVersion> GetOutputAsync(ProjectState project, HostDocument hostDocument, CancellationToken cancellationToken)
    {
        var document = project.Documents[hostDocument.FilePath];
        return GetOutputAsync(project, document, cancellationToken);
    }

    private static ValueTask<OutputAndVersion> GetOutputAsync(ProjectState project, DocumentState document, CancellationToken cancellationToken)
    {
