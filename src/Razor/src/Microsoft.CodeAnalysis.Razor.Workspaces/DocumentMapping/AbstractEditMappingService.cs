    ...
    IFilePathService filePathService) : IEditMappingService
{
    ...
    private async Task<Dictionary<string, TextEdit[]>> RemapDocumentEditsAsync(IRazorDocument contextDocument, Dictionary<string, TextEdit[]> changes, CancellationToken cancellationToken)
    {
        ...
        foreach (var (uriString, edits) in changes)
        {
            ...
            if (!TryGetDocumentContext(contextDocument, uri, projectContext: null, out var documentContext))
            {
                continue;
            }
