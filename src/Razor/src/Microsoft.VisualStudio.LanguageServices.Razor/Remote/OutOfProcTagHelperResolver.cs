    ...
    ITelemetryReporter telemetryReporter) : ITagHelperResolver
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await ResolveTagHelpersOutOfProcessAsync(roslynProject, project, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            return await ResolveTagHelpersInProcessAsync(roslynProject, project, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, $"Error encountered from project '{project.FilePath}':{Environment.NewLine}{ex}");
    ...
    protected virtual async ValueTask<ImmutableArray<TagHelperDescriptor>> ResolveTagHelpersOutOfProcessAsync(Project roslynProject, RazorProject project, CancellationToken cancellationToken)
    {
        if (!_resultCache.TryGetId(roslynProject.Id, out var lastResultId))
        {
            lastResultId = -1;
        }

        var projectHandle = new ProjectSnapshotHandle(roslynProject.Id, project.Configuration, project.RootNamespace);
        ...
        var checksums = ProduceChecksumsFromDelta(roslynProject.Id, lastResultId, deltaResult);
    ...
    protected virtual ValueTask<ImmutableArray<TagHelperDescriptor>> ResolveTagHelpersInProcessAsync(
        ...
        RazorProject project,
        CancellationToken cancellationToken)
        => roslynProject.GetTagHelpersAsync(project.ProjectEngine, _telemetryReporter, cancellationToken);
}
