        ...
        try
        {
            return _projectManager.UpdateAsync(
                static async (updater, state) =>
                {
                    ...
                    {
                        // The user is opening a document that is part of a fallback project.
                        // This is a scenario we are very interested in knowing more about
                        // so fire some telemetry. We can't log details about the project, for
                        // PII reasons, but we can use document count and tag helper count
                        ...
                        telemetryReporter.ReportEvent(
                            ...
                            Severity.Normal,
                            new Property("document.count", ((ProjectSnapshot)project).DocumentCount),
