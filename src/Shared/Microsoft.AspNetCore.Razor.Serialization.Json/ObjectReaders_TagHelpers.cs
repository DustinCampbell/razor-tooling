...
internal static partial class ObjectReaders
{
...
#endif
    {
        ...
        var kind = (TagHelperKind)reader.ReadInt32OrZero(nameof(TagHelperDescriptor.Kind));
        var runtimeKind = (RuntimeKind)reader.ReadInt32OrZero(nameof(TagHelperDescriptor.RuntimeKind));
        ...
        var typeName = reader.ReadStringOrNull(nameof(TagHelperDescriptor.TypeName));
        var typeNamespace = reader.ReadStringOrNull(nameof(TagHelperDescriptor.TypeNamespace));
        var typeNameIdentifier = reader.ReadStringOrNull(nameof(TagHelperDescriptor.TypeNameIdentifier));
        var displayName = reader.ReadStringOrNull(nameof(TagHelperDescriptor.DisplayName));
        var flags = (TagHelperFlags)reader.ReadInt32OrZero(nameof(TagHelperDescriptor.Flags));
        ...
        tagHelper = new TagHelperDescriptor(
            kind, runtimeKind, Cached(name), Cached(assemblyName),
            Cached(typeName), Cached(typeNamespace), Cached(typeNameIdentifier),
            Cached(displayName)!, flags, documentationObject, Cached(tagOutputHint),
        ...
        static BoundAttributeDescriptor ReadBoundAttribute(JsonDataReader reader)
        {
            ...
            {
                var kind = (TagHelperKind)reader.ReadInt32OrZero(nameof(BoundAttributeDescriptor.Kind));
                ...
                var flags = (BoundAttributeFlags)reader.ReadInt32OrDefault(nameof(BoundAttributeDescriptor.Flags), defaultValue: (int)BoundAttributeFlags.CaseSensitive);
        ...
        static BoundAttributeParameterDescriptor ReadBoundAttributeParameter(JsonDataReader reader)
        {
            ...
            {
                var kind = (TagHelperKind)reader.ReadInt32OrZero(nameof(BoundAttributeParameterDescriptor.Kind));
                ...
                var flags = (BoundAttributeParameterFlags)reader.ReadInt32OrDefault(nameof(BoundAttributeParameterDescriptor.Flags), defaultValue: (int)BoundAttributeParameterFlags.CaseSensitive);
                ...
                return new BoundAttributeParameterDescriptor(
                    kind, Cached(name)!, Cached(typeName),
                    flags, documentationObject, Cached(displayName),
                    metadata, diagnostics);
            }
        }
