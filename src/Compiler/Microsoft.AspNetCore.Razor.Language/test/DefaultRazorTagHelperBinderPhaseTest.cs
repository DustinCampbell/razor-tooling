...
";
    ...
    private static TagHelperDescriptor CreateComponentDescriptor(
        ...
        IEnumerable<Action<BoundAttributeDescriptorBuilder>> attributes = null,
        IEnumerable<Action<TagMatchingRuleDescriptorBuilder>> ruleBuilders = null,
        TagHelperKind? kind = null,
        bool fullyQualified = false,
        bool childContent = false)
    {
        kind ??= childContent ? TagHelperKind.ChildContent : TagHelperKind.Component;
        return CreateDescriptor(kind.GetValueOrDefault(), tagName, typeName, assemblyName, typeNamespace, typeNameIdentifier, attributes, ruleBuilders, fullyQualified);
    }
    #endregion
        ...
        bool componentFullyQualified = false)
    {
        ...
        builder.TypeNamespace = typeNamespace ?? (typeName.LastIndexOf('.') == -1 ? "" : typeName[..typeName.LastIndexOf('.')]);
        builder.TypeNameIdentifier = typeNameIdentifier ?? (typeName.LastIndexOf('.') == -1 ? typeName : typeName[(typeName.LastIndexOf('.') + 1)..]);
