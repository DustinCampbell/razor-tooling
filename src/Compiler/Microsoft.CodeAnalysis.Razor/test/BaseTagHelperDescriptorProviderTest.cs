...
public abstract class TagHelperDescriptorProviderTestBase
{
        ...
        int expectedCount)
    {
        ...
        var fullyQualifiedNameMatchComponents = components.Where(c => c.UseFullyQualifiedNameMatch).ToArray();
        Assert.Equal(expectedCount, fullyQualifiedNameMatchComponents.Length);

        var shortNameMatchComponents = components.Where(c => !c.UseFullyQualifiedNameMatch).ToArray();
